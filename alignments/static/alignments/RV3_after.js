...
function populateMSAIndices(permutation_checked) {
    if (permutation_checked) {
        vm.msaLength = getMSALength();
        document.getElementById('permutation_indices').value='1-' + vm.msaLength;
    }
}
...
function validatePermutationIndices() {
    let pass = () => {
        document.getElementById("downloadPermutationButton").disabled = false;
        document.getElementById("invalidPermutationIndicesMessage").style.display = "none";
    };
    let fail = () => {
        document.getElementById("downloadPermutationButton").disabled = true;
        document.getElementById("invalidPermutationIndicesMessage").style.display = "block";
    };
    let permutationIndices = document.getElementById("permutation_indices").value;
    ...
    for (let i = 0; i < splitOnCommas.length; i++) {
        ...
        if (dashIndex >= 0) {
            ...
            let endMSAIndex = parseInt(splitOnCommasI.substring(dashIndex));
            if (startMSAIndex < 1 || startMSAIndex > msaLength) {
                fail();
                return;
            }
            if (endMSAIndex < 1 || endMSAIndex > msaLength) {
                fail();
                return;
            }
        } else {
            let msaIndex = parseInt(splitOnCommasI);
            if (msaIndex < 1 || msaIndex > msaLength) {
                fail();
                return;
            }
        }
...
function handlePermutation() {
    let permutation_indices = document.getElementById("permutation_indices").value;
